...
placed on most distanced block from astronaut.
Goal for Saturn is simple, reach the finish by crossing terrain using wasd keys or arrow keys for moving and space button for teleporting in 2d space before time runs.
While on the other hand, opposing player (Jupyter god of space) wins if time runs out before astronaut reaches finish.
Jupyter have 2 abilities, first one being ability to arise mentioned blocks and by doing that "walls" are created on terrain which forces Saturn to go arround them.
...
Jupyter can never trap astronaut in walls and make reaching finish impossible because his goal is not to make reaching finish for astronaut imposible, but harder (there is always one path that leads to finish).
